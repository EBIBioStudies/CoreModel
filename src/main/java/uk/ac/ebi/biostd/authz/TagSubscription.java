...
@Entity
@NamedQueries({
        @NamedQuery(name = TagSubscription.GetUsersByTagIdsQuery,
        ...
        @NamedQuery(name = TagSubscription.GetSubsByTagIdsQuery,
                query = "SELECT ts FROM TagSubscription ts where ts.tag.id in (:" +
                        TagSubscription.TagIdQueryParameter + ")"), //  order by ts.user.id
        ...
        @NamedQuery(name = TagSubscription.GetByTagAndUserQuery,
                query = "SELECT ts FROM TagSubscription ts LEFT JOIN ts.user u LEFT JOIN ts.tag tg " +
                        ...
                        " AND tg.name=:" + TagSubscription.TagNameQueryParameter +
                        " AND u.id=:" + TagSubscription.UserIdQueryParameter),

        @NamedQuery(name = TagSubscription.GetByTagIdAndUserQuery,
                query = "SELECT ts FROM TagSubscription ts LEFT JOIN ts.user u LEFT JOIN ts.tag tg  " +
                        "where tg.id=:" + TagSubscription.TagIdQueryParameter + " AND u.id=:" +
                        TagSubscription.UserIdQueryParameter),

        @NamedQuery(name = TagSubscription.GetAllByUserQuery,
                query = "SELECT ts FROM TagSubscription ts LEFT JOIN ts.user u " +
